class SettingsForm
  ...
  def self.load(account)
    new(
      weights: ScoringWeight::CRITERIA.index_with { |criterion| account.scoring_weights.find_by(criterion: criterion)&.weight || 0 },
  ...
  def self.reset_to_defaults!(account:, updated_by: nil)
    ActiveRecord::Base.transaction do
      DEFAULT_WEIGHTS.each do |criterion, weight|
        account.scoring_weights.find_or_initialize_by(criterion: criterion).update!(weight: weight, updated_by: updated_by)
      end
      DEFAULT_RULES.each do |key, attrs|
        account.assignment_rules.find_or_initialize_by(key: key)
      ...
      RiskThreshold::DEFAULTS.each do |key, multiplier|
        account.risk_thresholds.find_or_initialize_by(key: key).update!(multiplier: multiplier, enabled: true, updated_by: updated_by)
      end
      AssignmentSetting.for(account).update!(
  ...
  def save(params, account:, updated_by: nil)
    ActiveRecord::Base.transaction do
      ...
      AssignmentRule::KEYS.keys.each do |key|
        ...
        rule = account.assignment_rules.find_or_initialize_by(key: key) do |new_rule|
      ...
      RiskThreshold::DEFAULTS.keys.each do |key|
        ...
        threshold = account.risk_thresholds.find_or_initialize_by(key: key)
