...
RSpec.describe Scoring::Ranking, type: :model do
  ...
  let(:event) { create(:event, account: account, starts_at: Time.zone.parse("2026-08-12 09:00"), ends_at: Time.zone.parse("2026-08-12 11:00")) } # Wed

  def assign_event!(liaison, starts_at:)
    e = create(:event, account: account, starts_at: starts_at, ends_at: starts_at + 2.hours)
    e.assign_to!(liaison, by: admin)
  end

  before do
    create(:assignment_setting, account: account, work_weeks_per_year: 46, weekly_target: 2)
  end
  ...
  describe "weighting and ranking" do
    before do
      ...
      create(:scoring_weight, account: account, criterion: "weekly_load_balance", weight: 100)
    end

    it "ranks liaisons with more room this week above those closer to pace" do
      fresh = create(:liaison, account: account)
      busy = create(:liaison, account: account)
  ...
  describe "hard-rule blocking" do
    before do
      create(:scoring_weight, account: account, criterion: "weekly_load_balance", weight: 100)
      create(:assignment_rule, account: account, key: "max_weekly_events", threshold: 1, enabled: true)
    end
    ...
    it "sorts blocked candidates after eligible ones regardless of score" do
      blocked_liaison = create(:liaison, account: account)
      assign_event!(blocked_liaison, starts_at: Time.zone.parse("2026-08-10 09:00"))
      eligible_liaison = create(:liaison, account: account)
  ...
  describe "load holds" do
    ...
    it "blocks a liaison whose active load hold covers this event" do
      liaison = create(:liaison, account: account)
      weekend_event = create(:event, :weekend, account: account)
